...
**`main`의 미배포 변경:** 워커가 파일을 직접 검색·읽으며 `trackrun`은 Orca, 설정된 터미널 실행기 또는 기존 tmux 세션에서 실시간 로그를 우선 표시합니다. 터미널이 없으면 헤드리스로 실행하고 `--launcher headless`로 명시할 수도 있습니다. 공개된 `0.0.1` wheel에는 아직 이전 스냅샷·헤드리스 방식이 들어 있습니다. [실행 방식과 경로 전달](OPERATIONS.md#worker-context-and-terminal-launchers-unreleased)을 참고하세요.
