...
![TODO에서 결과까지 — 익명 선행 운영 사례의 일평균 커밋 활동 33배](assets/metrics/workflow-impact.png)

**등록하고, 고르고, 실행하세요.** 사람은 방향을 정하고, 에이전트는 구현·검증·리뷰·허용된 랜딩까지 작업을 이어갑니다.

- **여러 작업을 동시에.** 트랙마다 독립된 워크트리에서 병렬로 진행합니다.
- **세션이 끊겨도 이어서.** 다음 워커가 파일에 남은 상태와 근거를 읽고 이어받습니다.
- **진행 상황은 한눈에.** 현재 작업과 대기 이유, 결과를 대시보드에서 확인합니다.

익명 선행 운영 사례에서 **일평균 커밋 9.3 → 316.3건**이 관측됐습니다. 1월 대비 9월의 커밋 활동 기준이며, 9월은 1~23일 실측입니다.
...
6월 대비 9월의 보정 소스 변화량/일은 **4.52배**, 7월 대비 완료 전환/일은 **2.30배**입니다. 1월 대비 소스 변화량/일은 1.09배이며, 9월 완료 전환/일은 8월보다 낮았습니다. 커밋 증가와 코드·완료 처리량의 증가를 같은 숫자로 표현하지 않습니다.

![월별 소스 추가·삭제와 생성물·이동 등의 제외량](assets/metrics/source-changes.png)

현재 패키지의 벤치마크와 구분되는 선행 운영 기록입니다. 생성물·외부 소스·알려진 초기 입력 및 저장소 이동 영향을 소스 지표에서 제외했습니다. 관측 증가율이며, 투입 시간이나 다른 조건을 통제한 생산성 실험은 아닙니다.
